...
from .sam3_symbols import (
    ...
    SCHEMA_VERSION as SYMBOLS_SCHEMA_VERSION,
    empty_doc as empty_symbols_doc,
    grid_matches_plan as symbols_grid_matches_plan,
)
from .symbols_pipeline import (
...
def process_session(
    ...
    *,
    ...
    (out / "plan.json").write_text(json.dumps(plan, indent=2))
    # Drop the bundled example symbols.json into the session if one exists
    # for this session id and the user hasn't generated their own. Keeps
    # the demo "click to see symbols" flow one-step for the canned scan.
    maybe_preload_symbols(session_id, sd)
    return plan
...
async def api_pdf(session_id: str) -> Response:
    ...
    *edge* stroked exactly once. Pre-snap (no topology) falls back to the
    ...
    # ─── Ceiling-services symbols (drawn on top of fills + edges) ──
    # Black outlines on a white fill so each fixture reads through the
    # ceiling tints. Pulled from sessions/<id>/out/symbols.json — the
    # file is regenerated by the SAM 3 + projection pipeline.
    sym_doc = read_session_symbols(_session_dir(session_id))
    sym_classes = (sym_doc or {}).get("symbol_classes") or SYMBOL_CLASSES
    if sym_doc and (sym_doc.get("symbols") or []):
        _draw_symbols_layer(ax, sym_doc, scale_ratio=scale_ratio)
...
# Class identity travels via the legend swatch + label colour, not the
# body of the marker, so a busy plan reads cleanly under the ceiling
# tints. The same convention is mirrored in the canvas overlay
# (drawSymbolsOverlay in static/app.js).

SYMBOL_PDF_STROKE = "#111111"
SYMBOL_PDF_FILL = "#ffffff"
SYMBOL_PDF_LINE_WIDTH = 0.6   # matplotlib points; reads ~0.2mm at A1


def _symbol_px_to_world(u: float, v: float, grid: dict) -> tuple[float, float]:
    """Mirror of static/app.js worldToImg/imgToWorld — pixel (u, v) on
    the ortho grid → world (x, z) in metres. Both X and V/Z are
    inverted (RCP convention)."""
    ppm = float(grid["pixels_per_metre"])
    x = float(grid["max_x"]) - u / ppm
    z = float(grid["max_z"]) - v / ppm
    return x, z


def _symbol_rect_corners_world(
    sym: dict, def_: dict, grid: dict,
) -> list[tuple[float, float]]:
    """Return the four corners of a rotated-rect symbol in world coords.
    Honours the per-symbol angle_deg + size with a fall-back to the
    class default size when length_px / width_px are zero (e.g.
    hand-added markers without a backing cluster)."""
    import math as _m
    ppm = float(grid["pixels_per_metre"])
    px_per_cm = ppm / 100.0
    min_px = 8.0
    length_px = max(min_px, float(sym.get("length_px") or 0.0)
                    or (def_.get("default_size_cm") or [30.0, 30.0])[0] * px_per_cm)
    width_px = max(min_px, float(sym.get("width_px") or 0.0)
                   or (def_.get("default_size_cm") or [30.0, 30.0])[1] * px_per_cm)
    cu, cv = float(sym["centroid_px"][0]), float(sym["centroid_px"][1])
    a = _m.radians(float(sym.get("angle_deg") or 0.0))
    cosA, sinA = _m.cos(a), _m.sin(a)
    hl, hw = length_px / 2.0, width_px / 2.0
    pts_px = [(-hl, -hw), (hl, -hw), (hl, hw), (-hl, hw)]
    out = []
    for x_, y_ in pts_px:
        u_p = cu + x_ * cosA - y_ * sinA
        v_p = cv + x_ * sinA + y_ * cosA
        out.append(_symbol_px_to_world(u_p, v_p, grid))
    return out


def _draw_symbols_layer(ax, sym_doc: dict, *, scale_ratio: int) -> None:
    """Draw every symbol in ``sym_doc`` onto the plan axes (which are in
    world metres). Sizes use the class-defined cm units so circles stay
    crisp regardless of the per-symbol footprint variance.

    ``scale_ratio`` is the architectural scale (e.g. 50 for 1:50). It's
    used to size the rect_with_M label so the M sits inside the box on
    paper — matplotlib font sizes are in points, so the conversion goes
    cm-of-world → mm-of-paper → points.
    """
    from matplotlib.patches import (
        Circle as MplCircle, Polygon as MplPolygon,
    )
    grid = sym_doc.get("grid") or {}
    if not grid:
        return
    classes = sym_doc.get("symbol_classes") or SYMBOL_CLASSES
    # cm of paper per cm of world. At 1:50, 1cm world = 0.02cm paper.
    paper_cm_per_world_cm = 1.0 / float(scale_ratio)

    for sym in (sym_doc.get("symbols") or []):
        def_ = classes.get(sym.get("class"))
        if not def_:
            continue
        shape = def_.get("shape")
        cu, cv = float(sym["centroid_px"][0]), float(sym["centroid_px"][1])
        wx, wz = _symbol_px_to_world(cu, cv, grid)

        if shape == "circle":
            r_m = float(def_.get("fixed_diameter_cm") or 20.0) / 200.0
            ax.add_patch(MplCircle(
                (wx, wz), radius=r_m,
                facecolor=SYMBOL_PDF_FILL, edgecolor=SYMBOL_PDF_STROKE,
                linewidth=SYMBOL_PDF_LINE_WIDTH, zorder=5,
            ))
        elif shape == "dot_in_circle":
            r_o = float(def_.get("fixed_diameter_cm") or 12.0) / 200.0
            r_i = float(def_.get("fixed_inner_dot_diameter_cm") or 5.0) / 200.0
            ax.add_patch(MplCircle(
                (wx, wz), radius=r_o,
                facecolor=SYMBOL_PDF_FILL, edgecolor=SYMBOL_PDF_STROKE,
                linewidth=SYMBOL_PDF_LINE_WIDTH, zorder=5,
            ...
            ax.add_patch(MplCircle(
                (wx, wz), radius=r_i,
                facecolor=SYMBOL_PDF_STROKE, edgecolor=SYMBOL_PDF_STROKE,
                linewidth=0, zorder=6,
            ))
        elif shape in ("rect", "rect_with_cross", "rect_with_M"):
            corners = _symbol_rect_corners_world(sym, def_, grid)
            ax.add_patch(MplPolygon(
                corners, closed=True,
                facecolor=SYMBOL_PDF_FILL, edgecolor=SYMBOL_PDF_STROKE,
                linewidth=SYMBOL_PDF_LINE_WIDTH, zorder=5,
            ))
            if shape == "rect_with_cross":
                # Diagonals between opposite corners (matches the
                # cv2.boxPoints / canvas rendering convention).
                a, b, c, d = corners
                ax.plot([a[0], c[0]], [a[1], c[1]],
                        color=SYMBOL_PDF_STROKE,
                        linewidth=SYMBOL_PDF_LINE_WIDTH, zorder=6)
                ax.plot([b[0], d[0]], [b[1], d[1]],
                        color=SYMBOL_PDF_STROKE,
                        linewidth=SYMBOL_PDF_LINE_WIDTH, zorder=6)
            elif shape == "rect_with_M":
                # Paper size of the rect's shorter side, in points. The
                # M wants to sit at ~50% of that. font size 4–6 pt is
                # the legible sweet spot at A1.
                ppm = float(grid["pixels_per_metre"])
                short_side_m = (
                    min(float(sym.get("length_px") or 0.0),
                        float(sym.get("width_px") or 0.0)) / ppm
                )
                if short_side_m <= 0:
                    short_side_m = (
                        min(*(def_.get("default_size_cm") or [30.0, 30.0])) / 100.0
                    )
                short_side_paper_mm = short_side_m * 1000.0 / float(scale_ratio)
                fontpt = max(3.0, min(7.0, short_side_paper_mm * 1.5))
                ax.text(wx, wz, "M",
                        ha="center", va="center",
                        fontsize=fontpt, fontweight="bold",
                        color=SYMBOL_PDF_STROKE, zorder=6)


def _draw_symbol_legend_marker(ax, x: float, y: float, w: float, h: float,
                                def_: dict) -> None:
    """Draw a tiny exemplar of a symbol class inside a legend cell. All
    coordinates are in the legend axis's 0–1 space."""
    from matplotlib.patches import (
        Circle as MplCircle, Rectangle as MplRect,
    )
    cx, cy = x + w / 2.0, y + h / 2.0
    shape = def_.get("shape")
    color = SYMBOL_PDF_STROKE
    fill = SYMBOL_PDF_FILL
    lw = SYMBOL_PDF_LINE_WIDTH
    if shape == "circle":
        r = min(w, h) * 0.40
        ax.add_patch(MplCircle((cx, cy), r, facecolor=fill, edgecolor=color, linewidth=lw))
    elif shape == "dot_in_circle":
        r_o = min(w, h) * 0.40
        r_i = r_o * 0.45
        ax.add_patch(MplCircle((cx, cy), r_o, facecolor=fill, edgecolor=color, linewidth=lw))
        ax.add_patch(MplCircle((cx, cy), r_i, facecolor=color, edgecolor=color, linewidth=0))
    else:
        rw, rh = w * 0.62, h * 0.62
        rx, ry = cx - rw / 2.0, cy - rh / 2.0
        ax.add_patch(MplRect((rx, ry), rw, rh, facecolor=fill, edgecolor=color, linewidth=lw))
        if shape == "rect_with_cross":
            ax.plot([rx, rx + rw], [ry, ry + rh], color=color, linewidth=lw)
            ax.plot([rx, rx + rw], [ry + rh, ry], color=color, linewidth=lw)
        elif shape == "rect_with_M":
...
def _draw_legends(legend_ax, *, main: dict | None, regions: list,
                  obstructions: list, units: str,
                  symbols_doc: dict | None = None) -> None:
    ...
    # Services column — one row per ceiling-fixture class actually
    # present in the symbols.json. Empty classes are dropped so the
    # legend stays focused on what's on the plan; if no symbols exist
    # the placeholder reads "no symbols generated".
    legend_ax.text(0.805, 0.96, "SERVICES",
                   ha="left", va="top",
                   fontsize=8, fontweight="bold", color="#888")
    sym_classes = (symbols_doc or {}).get("symbol_classes") or SYMBOL_CLASSES
    sym_counts: dict[str, int] = {}
    for s in (symbols_doc or {}).get("symbols", []) or []:
        k = s.get("class")
        if k:
            sym_counts[k] = sym_counts.get(k, 0) + 1
    sym_rows = [
        (k, def_, sym_counts.get(k, 0))
        for k, def_ in sym_classes.items()
        if sym_counts.get(k, 0) > 0
    ]
    if sym_rows:
        legend_ax.text(0.81, 0.92,
                       f"{sum(sym_counts.values())} fixtures, "
                       f"{len(sym_rows)} classes",
                       ha="left", va="top", fontsize=7, color="#666",
                       style="italic")
        # Vertical layout — fits up to ~6 rows in the 0.85-tall services
        # cell. The 5 default classes always fit; future additions would
        # need a second column.
        n = len(sym_rows)
        row_h = min(0.13, 0.78 / max(1, n))
        marker_w, marker_h = 0.025, 0.05
        text_x = 0.84 + marker_w
        for i, (key, def_, count) in enumerate(sym_rows):
            y = 0.84 - (i + 1) * row_h
            _draw_symbol_legend_marker(
                legend_ax, 0.815, y - 0.005, marker_w, marker_h, def_,
            )
            legend_ax.text(text_x, y + marker_h / 2 + 0.008,
                           f"{def_.get('label', key)}",
                           ha="left", va="center", fontsize=7, color="#222")
            legend_ax.text(text_x, y - 0.005,
                           f"{count}",
                           ha="left", va="bottom", fontsize=6, color="#666")
    else:
        legend_ax.add_patch(Rectangle(
            (0.81, 0.05), 0.18, 0.85,
...
#
# The optional symbols.json document lives at sessions/<id>/out/symbols.json
# and is produced by the experimental SAM3 + projection pipeline (see
# `symbols_pipeline` for the orchestration plan). The viewer treats it as
# overlay metadata: read for display, write for hand-edits, regenerate
# from the source scan via the generate endpoint.

@app.get("/api/symbol_classes")
async def api_symbol_classes() -> dict:
    """Static class definitions used by the viewer's legend / picker. The
    same dict is embedded in every per-session symbols.json so a user can
    open a hand-edited file later without the package available."""
    return {
...
async def api_get_symbols(session_id: str) -> dict:
    sd = _session_dir(session_id)
    plan = _load_plan(session_id)
    # If a bundled example symbols.json exists for this session id and
    # nothing is on disk yet, drop it in. Lets the demo session light up
    # without forcing the user to re-process their upload.
    maybe_preload_symbols(session_id, sd)
    doc = read_session_symbols(sd)
    if doc is None:
        # Empty doc keyed to the current grid so the viewer has something
        # to anchor the legend / pipeline-status UI against even before
        # detection has run.
        grid = plan.get("grid") or {}
        from .planes import PlanGrid as _PG
        if grid:
            pg = _PG(
            ...
            return {**empty_symbols_doc(grid=pg, session_id=session_id),
                    "status": "absent"}
        return {"version": SYMBOLS_SCHEMA_VERSION,
                "session_id": session_id,
                "symbol_classes": SYMBOL_CLASSES,
                "symbols": [],
                "status": "absent"}
    # Flag a grid mismatch so the viewer can warn the user — re-rendering
    # the ortho with new scan settings invalidates the pixel coordinates
    # the symbols are anchored to.
    doc["status"] = (
        "ok" if symbols_grid_matches_plan(doc, plan) else "stale_grid"
    )
    return doc


@app.put("/api/sessions/{session_id}/symbols")
async def api_put_symbols(session_id: str, payload: dict = Body(...)) -> dict:
    """Save a user-edited symbols document. Minimal validation — the
    viewer is the only writer right now and we trust its shape."""
    sd = _session_dir(session_id)
    if not isinstance(payload, dict) or "symbols" not in payload:
        raise HTTPException(400, "payload must be a symbols document")
    payload.setdefault("version", SYMBOLS_SCHEMA_VERSION)
    payload.setdefault("session_id", session_id)
    payload.setdefault("symbol_classes", SYMBOL_CLASSES)
    write_session_symbols(sd, payload)
    return {"ok": True, "n_symbols": len(payload.get("symbols") or [])}


@app.delete("/api/sessions/{session_id}/symbols")
async def api_delete_symbols(session_id: str) -> dict:
    sd = _session_dir(session_id)
    p = session_symbols_path(sd)
    if p.exists():
        p.unlink()
...
async def api_generate_symbols(
    ...
    """Kick off the SAM 3 → projection pipeline for this session.

    Stubbed: returns 501 with an actionable message. The two backends
    (local MPS/CUDA, RunPod GPU) live as standalone experiment scripts;
    one-button orchestration is the next milestone."""
    sd = _session_dir(session_id)
    backend = (payload.get("backend") or "local").lower()
    try:
        result = generate_symbols_for_session(session_id, sd, backend=backend)
    except PipelineNotImplemented as e:
        # 501 = the server understands the request but the feature isn't
        # implemented. The viewer surfaces ``detail`` in the banner.
